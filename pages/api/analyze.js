...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!EXPLORER_API_BASE) {
      return res.status(500).json({ error: "EXPLORER_API_BASE not configured" });
    }
    ...
    console.log("Fetching Explorer API:", apiUrl);

    const r = await fetch(apiUrl);

    if (!r.ok) {
      const text = await r.text();
      console.error("Explorer Fetch Failed:", text);
      return res.status(502).json({ error: "Explorer fetch failed", details: text });
    }

    const json = await r.json();
    const txs = Array.isArray(json.result)
      ? json.result
      : json.txs || json.transactions || [];

    // --- Analisis Wallet ---
    const timestamps = txs
      .map((t) => Number(t.timeStamp || t.timestamp || 0))
      .filter(Boolean);
    ...
    txs.forEach((t) => {
      ...
      if (input && (input.startsWith("0x38ed1739") || input.includes("swap"))) kinds.add("swap");
      if (input && input.startsWith("0x095ea7b3")) kinds.add("approve");
      if (t.to) kinds.add("transfer");
      if (input && input.length > 10) kinds.add("contract-interact");
    });

    const activityDiversity =
      kinds.size >= 3 ? "high" : kinds.size === 2 ? "medium" : kinds.size === 1 ? "low" : "none";
    ...
    for (let i = 0; i < sorted.length; i++) {
      let j = i,
        count = 0;
      while (j < sorted.length && sorted[j] - sorted[i] <= 3600) {
        count++;
        j++;
      }
    ...
    // Skoring
    let score = 0;
    if (["wormhole/bridge", "faucet"].includes(fundingSource)) score += 0;
    else if (fundingSource === "unknown") score += 1;
    else score += 2;

    if (sharedFunder) score += sharedFunderCount >= 10 ? 2 : 1;
    if (activityDiversity === "high") score += 0;
    else if (activityDiversity === "medium") score += 1;
    else score += 2;

    if (timingPattern === "distributed") score += 0;
    else if (timingPattern === "some-cluster") score += 1;
    else score += 2;

    if (findings.walletAgeDays >= 14) score += 0;
    else if (findings.walletAgeDays >= 3) score += 1;
    else score += 2;

    const riskLevel = score <= 3 ? "Low" : score <= 6 ? "Medium" : "High";

    const recommendations = [];
    if (findings.fundingSource && !["wormhole/bridge", "faucet"].includes(findings.fundingSource))
      recommendations.push("Pertimbangkan menggunakan bridge resmi (Wormhole) atau faucet resmi.");
